...
// PendingMembersHandleRequest - 許可待ちメンバーリストのリクエストハンドラ
func PendingMembersHandleRequest(ctx context.Context,
	...
	// Logic呼び出し
	logic := newPendingMembersLogic(repository.NewMemberRepository(), repository.NewAdminRepository())
...
func main() {
	lambda.Start(PendingMembersHandleRequest)
}
