...
func (m *mockMemberRepository) SelectByRoomIDAndState(roomID int, state int) (*[]model.Member, error) {
	members := []model.Member{
		{UserID: "user1", RoomID: 42, State: enum.PENDING},
		{UserID: "user2", RoomID: 42, State: enum.PENDING},
		{UserID: "user3", RoomID: 42, State: enum.PENDING},
	}
	return &members, nil
}

func TestHandle(t *testing.T) {
	logic := newPendingMembersLogic(newMockMemberRepository(), newMockAdminRepository())
	res, err := logic.handle("user1", 42)
	if err != nil {
		t.Fatal(err)
	}

	expect := &response.PendingMemberResponse{
